...
/*   Updated: 2020/01/21 19:47:00 by mbos        ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static t_bool	walls_cases(t_rays temp, t_mlx *mlx)
{
	if (temp.facing_up && temp.wall.hz)
		display_wall_hz(temp, mlx->map, mlx->map->north, mlx);
	else if (temp.facing_down && temp.wall.hz)
		display_wall_hz(temp, mlx->map, mlx->map->south, mlx);
	else if (temp.facing_left && temp.wall.vt)
		display_wall_vt(temp, mlx->map, mlx->map->west, mlx);
	else if (temp.facing_right && temp.wall.vt)
		display_wall_vt(temp, mlx->map, mlx->map->east, mlx);
	return (True);
}
