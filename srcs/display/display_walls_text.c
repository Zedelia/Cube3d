...
/*   Updated: 2020/01/21 19:47:47 by mbos        ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
t_bool	display_wall_hz(t_rays r, t_map *map, t_img img, t_mlx *mlx)
{
	...
	if (r.distance == 0 || (top - bottom) == 0)
		return (return_false(__func__, "[FAIL] can't divide by 0"));
	y = top;
	display_roof(mlx, r.x_id, top);
	while (y < bottom)
	{
		offset.y = (y - top) * ((double)img.height / (bottom - top));
		ft_pixel_put(mlx, r.x_id, y, ft_pixel_get_color(img, offset.x, offset.y));
		y++;
	}
	display_floor(mlx, r.x_id, bottom);
	return (True);
}
