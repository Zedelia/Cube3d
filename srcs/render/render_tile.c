...
t_bool render_color_cub(t_mlx *mlx, t_map *map, int l, int c, int color)
{
	int index_x = 0;
	int index_y = 0;
	int tile_size;
...
t_bool	render_tile(t_mlx *mlx, t_map *map)
{
	...
	while (l < map->map_lines)
	{
		...
		{
			if (map->tab[l][c] == 1)
				render_color_cub(mlx, map, l, c, 0xffba33);
			if (map->tab[l][c] == 0)
				render_color_cub(mlx, map, l, c, 0x33fff9);
			if (map->tab[l][c] == 2)
				render_color_cub(mlx, map, l, c, 0xff33fc);
			if (map->tab[l][c] > 2)
				render_color_cub(mlx, map, l, c, 0x4aff33);
			c++;
		}
		c = 0;
		l++;
